...
/**
 * file from <a href="https://github.com/InstantWebP2P/tweetnacl-java">https://github.com/InstantWebP2P/tweetnacl-java</a>
 * 
 * <br>TweetNacl.c Java porting
 * @author Copyright (c) 2014 Tom Zhou iwebpp@gmail.com
 */
public final class TweetNaclFast {
    ...
    public static final class Box {
        ...
        /*
         * @description Encrypt and authenticates message using peer's public key, our secret key, and the given nonce, which must be unique
         * for each distinct message for a key pair. Returns an encrypted and authenticated message, which is nacl.box.overheadLength longer
        ...
        /*
         * @description Encrypt and authenticates message using peer's public key, our secret key, and the given nonce, which must be unique
         * for each distinct message for a key pair. Explicitly pass the nonce Returns an encrypted and authenticated message, which is
        ...
        /*
         * @description Authenticates and decrypts the given box with peer's public key, our secret key, and the given nonce. Returns the
        ...
        /*
         * @description Authenticates and decrypts the given box with peer's public key, our secret key, and the given nonce. Explicit
         * passing of nonce Returns the original message, or null if authentication fails.
         */
        ...
        /*
         * @description Returns a precomputed shared key which can be used in nacl.box.after and nacl.box.open.after.
         */
        public byte[] before() {
            if (this.sharedKey == null) {
        ...
        /*
         * @description Same as nacl.box, but uses a shared key precomputed with nacl.box.before.
         */
        ...
        /*
         * @description Same as nacl.box, but uses a shared key precomputed with nacl.box.before, and passes a nonce explicitly.
         */
        ...
        /*
         * @description Same as nacl.box.open, but uses a shared key pre-computed with nacl.box.before.
         */
        ...
        /*
         * @description Generates a new random key pair for box and returns it as an object with publicKey and secretKey members:
         */
    ...
    public static final class SecretBox {
        ...
        /*
         * @description Encrypt and authenticates message using the key and the nonce. The nonce must be unique for each distinct message
         * for this key. Returns an encrypted and authenticated message, which is nacl.secretbox.overheadLength longer than the original
         * message.
         */
        public byte[] box(byte[] message) {
            if (message == null)
        ...
        /*
         * @description Authenticates and decrypts the given secret box using the key and the nonce. Returns the original message, or null
        ...
        /*
         * @description Length of overhead added to secret box compared to original message.
         */
        public static final int overheadLength = 16;
    ...
    public static final class ScalarMult {
        ...
        /*
         * @description Multiplies an integer n by a group element p and returns the resulting group element.
         */
        ...
        /*
         * @description Multiplies an integer n by a standard group element and returns the resulting group element.
         */
    ...
    public static final class Signature {
        ...
        /*
         * @description Signs the message using the secret key and returns a signed message.
         */
        ...
        /*
         * @description Verifies the signed message and returns the message without signature. Returns null if verification failed.
         */
        ...
        /*
         * @description Generates new random key pair for signing and returns it as an object with publicKey and secretKey members
         */
        public static class KeyPair {
            private byte[] publicKey;
            private byte[] secretKey;
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    /*
     * @description Codes below are ported tweetnacl-fast.js from TweetNacl.c/TweetNacl.h
     */
    ...
    private static void core_salsa20(byte[] o, byte[] p, byte[] k, byte[] c) {
        int j0 = c[0] & 0xff | (c[1] & 0xff) << 8 | (c[2] & 0xff) << 16 | (c[3] & 0xff) << 24, j1 = k[0] & 0xff | (k[1] & 0xff) << 8 | (k[2] & 0xff) << 16 | (k[3] & 0xff) << 24,
    ...
    /*
     * static { try { sigma = "expand 32-byte k".getBytes("utf-8"); } catch (UnsupportedEncodingException e) { e.printStackTrace(); } }
     */
    ...
    public static int crypto_stream_xor(byte[] c, int cpos, byte[] m, int mpos, long d, byte[] n, byte[] k) {
        ...
        /*
         * String dbgt = ""; for (int dbg = 0; dbg < n.length; dbg ++) dbgt += " "+n[dbg]; Log.d(TAG, "crypto_stream_xor, nonce -> "+dbgt);
         * dbgt = ""; for (int dbg = 0; dbg < k.length; dbg ++) dbgt += " "+k[dbg]; Log.d(TAG, "crypto_stream_xor, shk -> "+dbgt);
         */
    ...
    private static int crypto_onetimeauth(byte[] out, final int outpos, byte[] m, final int mpos, int n, byte[] k) {
        ...
        /*
         * String dbgt = ""; for (int dbg = 0; dbg < out.length-outpos; dbg ++) dbgt += " "+out[dbg+outpos]; Log.d(TAG,
    ...
    private static void M(long[] o, final int ooff, long[] a, final int aoff, long[] b, final int boff) {
        long v, c, t0 = 0, t1 = 0, t2 = 0, t3 = 0, t4 = 0, t5 = 0, t6 = 0, t7 = 0, t8 = 0, t9 = 0, t10 = 0, t11 = 0, t12 = 0, t13 = 0, t14 = 0, t15 = 0, t16 = 0, t17 = 0, t18 = 0, t19 = 0, t20 = 0, t21 = 0, t22 = 0, t23 = 0, t24 = 0,
                t25 = 0, t26 = 0, t27 = 0, t28 = 0, t29 = 0, t30 = 0, b0 = b[0 + boff], b1 = b[1 + boff], b2 = b[2 + boff], b3 = b[3 + boff], b4 = b[4 + boff], b5 = b[5 + boff], b6 = b[6 + boff], b7 = b[7 + boff], b8 = b[8 + boff],
    ...
    private static final long K[] = { 0x428a2f98d728ae22L, 0x7137449123ef65cdL, 0xb5c0fbcfec4d3b2fL, 0xe9b5dba58189dbbcL, 0x3956c25bf348b538L, 0x59f111f1b605d019L, 0x923f82a4af194f9bL, 0xab1c5ed5da6d8118L, 0xd807aa98a3030242L,
            0x12835b0145706fbeL, 0x243185be4ee4b28cL, 0x550c7dc3d5ffb4e2L, 0x72be5d74f27b896fL, 0x80deb1fe3b1696b1L, 0x9bdc06a725c71235L, 0xc19bf174cf692694L, 0xe49b69c19ef14ad2L, 0xefbe4786384f25e3L, 0x0fc19dc68b8cd5b5L,
            0x240ca1cc77ac9c65L, 0x2de92c6f592b0275L, 0x4a7484aa6ea6e483L, 0x5cb0a9dcbd41fbd4L, 0x76f988da831153b5L, 0x983e5152ee66dfabL, 0xa831c66d2db43210L, 0xb00327c898fb213fL, 0xbf597fc7beef0ee4L, 0xc6e00bf33da88fc2L,
            0xd5a79147930aa725L, 0x06ca6351e003826fL, 0x142929670a0e6e70L, 0x27b70a8546d22ffcL, 0x2e1b21385c26c926L, 0x4d2c6dfc5ac42aedL, 0x53380d139d95b3dfL, 0x650a73548baf63deL, 0x766a0abb3c77b2a8L, 0x81c2c92e47edaee6L,
            0x92722c851482353bL, 0xa2bfe8a14cf10364L, 0xa81a664bbc423001L, 0xc24b8b70d0f89791L, 0xc76c51a30654be30L, 0xd192e819d6ef5218L, 0xd69906245565a910L, 0xf40e35855771202aL, 0x106aa07032bbd1b8L, 0x19a4c116b8d2d0c8L,
            0x1e376c085141ab53L, 0x2748774cdf8eeb99L, 0x34b0bcb5e19b48a8L, 0x391c0cb3c5c95a63L, 0x4ed8aa4ae3418acbL, 0x5b9cca4f7763e373L, 0x682e6ff3d6b2b8a3L, 0x748f82ee5defb2fcL, 0x78a5636f43172f60L, 0x84c87814a1f0ab72L,
            0x8cc702081a6439ecL, 0x90befffa23631e28L, 0xa4506cebde82bde9L, 0xbef9a3f7b2c67915L, 0xc67178f2e372532bL, 0xca273eceea26619cL, 0xd186b8c721c0c207L, 0xeada7dd6cde0eb1eL, 0xf57d4f7fee6ed178L, 0x06f067aa72176fbaL,
            0x0a637dc5a2c898a6L, 0x113f9804bef90daeL, 0x1b710b35131c471bL, 0x28db77f523047d84L, 0x32caab7b40c72493L, 0x3c9ebe0a15c9bebcL, 0x431d67c49c100d4cL, 0x4cc5d4becb3e42b6L, 0x597f299cfc657e2aL, 0x5fcb6fab3ad6faecL,
            0x6c44198c4a475817L };
    ...
    private static int crypto_hashblocks_hl(int[] hh, int[] hl, byte[] m, final int moff, int n) {
        ...
        while (n >= 128) {
            ...
            /*
             * dbgt = ""; for (int dbg = 0; dbg < hh.length; dbg ++) dbgt += " "+hh[dbg]; Log.d(TAG, "\ncrypto_hashblocks_hl hh -> "+dbgt);
             * dbgt = ""; for (int dbg = 0; dbg < hl.length; dbg ++) dbgt += " "+hl[dbg]; Log.d(TAG, "\ncrypto_hashblocks_hl hl -> "+dbgt);
             */
        }
     ...
     * jrandom.nextLong(); x[i+0] = (byte) (rnd >>> 0); x[i+1] = (byte) (rnd >>> 8); x[i+2] = (byte) (rnd >>> 16); x[i+3] = (byte) (rnd >>>
     * 24); x[i+4] = (byte) (rnd >>> 32); x[i+5] = (byte) (rnd >>> 40); x[i+6] = (byte) (rnd >>> 48); x[i+7] = (byte) (rnd >>> 56); } if
     * (ret > 0) { rnd = jrandom.nextLong(); for (int i = len-ret; i < len; i ++) x[i] = (byte) (rnd >>> 8*i); } return x; }
     */
