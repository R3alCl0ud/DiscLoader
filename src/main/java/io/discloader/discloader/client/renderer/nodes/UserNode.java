...
public class UserNode extends DefaultMutableTreeNode {
	...
	public void updateNode(User data) {
		...
		if (data.username != null)
			this.username.setUserObject("username: " + data.username);
		if (data.discriminator != null)
			this.discriminator.setUserObject("discriminator: " + data.discriminator);
		if (data.avatar != null)
			this.avatar.setUserObject("avatar: " + data.avatar);
		if (data.bot || !data.bot)
			this.bot.setUserObject("bot: " + data.bot);
	}
