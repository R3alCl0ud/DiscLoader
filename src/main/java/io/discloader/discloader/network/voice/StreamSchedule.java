...
public class StreamSchedule extends AudioEventAdapter implements AudioLoadResultHandler, IVoiceConnectionListener {
	...
	public void shuffle() {
		...
		if (connection.player.getPlayingTrack() != null) {
			for (int i = tracks.size() - 2; i > 1; i--) {
				int n = (int) Math.floor(Math.random() * (i));
				if (n != 0) {
					AudioTrack temp = tracks.remove(i);
					tracks.add(i, tracks.remove(n));
					tracks.add(n, temp);
				} else {
					i++;
				}
	...
	public void onPlayerPause(AudioPlayer player) {
		if (b) this.connection.provider.close();
	}

	@Override
	public void onPlayerResume(AudioPlayer player) {
		this.connection.getWs().setSpeaking(true);
		if (b) this.connection.provider.start();
	}

	@Override
	public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) {
		if (b) {
			if (endReason == AudioTrackEndReason.FINISHED || endReason == AudioTrackEndReason.LOAD_FAILED) tracks.remove(track);
	...
	public void playlistLoaded(AudioPlaylist playlist) {
		if (b) {
			...
			for (IVoiceConnectionListener e : connection.listeners) {
				if (e != this) e.playlistLoaded(playlist);
			}
