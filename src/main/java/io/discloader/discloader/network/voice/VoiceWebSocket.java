...
public class VoiceWebSocket extends WebSocketAdapter {
	...
	public void onDisconnected(WebSocket ws, WebSocketFrame frame, WebSocketFrame frame2, boolean isServer)
			throws Exception {
		System.err.print(String.format("Reason: %s, Code: %d, isServer: %b", frame.getCloseReason(),
				frame.getCloseCode(), isServer));
	...
	public void sendIdentify() {
		VoiceIdentify payload = new VoiceIdentify(connection.guild.id, connection.loader.user.id,
				connection.getSessionID(), connection.getToken());
		...
		TimerTask heartbeat = new TimerTask() {
			...
			public void run() {
				if (ws.isOpen() && !connection.getUdpClient().udpSocket.isClosed()) {
					...
					System.err.print("Died: ws.isOpen(): " + ws.isOpen() + ", udp.isClosed(): "
