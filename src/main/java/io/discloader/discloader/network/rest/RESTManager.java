...
public class RESTManager {
	...
	public CompletableFuture<GuildMember> banMember(Guild guild, GuildMember member) {
		...
		this.makeRequest(Endpoints.guildBanMember(member.guild.id, member.id), Methods.PUT, true)
	...
	public CompletableFuture<TextChannel> createTextChannel(Guild guild, JSONObject data) {
		...
		this.makeRequest(DLUtil.Endpoints.guildChannels(guild.id), DLUtil.Methods.POST, true, data.put("type", "text"))
				.thenAcceptAsync(action -> {
					future.complete(
	...
	public CompletableFuture<VoiceChannel> createVoiceChannel(Guild guild, JSONObject data) {
		...
		this.makeRequest(DLUtil.Endpoints.guildChannels(guild.id), DLUtil.Methods.POST, true, data.put("type", "voice"))
				.thenAcceptAsync(action -> {
	...
	public CompletableFuture<Message> deleteMessage(ITextChannel channel, Message message) {
		...
		this.makeRequest(DLUtil.Endpoints.message(channel.getID(), message.id), DLUtil.Methods.DELETE, true)
				.thenAcceptAsync(action -> {
	...
	public CompletableFuture<Message> editMessage(ITextChannel channel, Message message, String content,
		...
		if (content.length() < 1 && (embed == null || attachment == null))
			return null;
		CompletableFuture<Message> future = new CompletableFuture<Message>();
		this.makeRequest(DLUtil.Endpoints.message(channel.getID(), message.id), DLUtil.Methods.PATCH, true,
	...
	public CompletableFuture<GuildMember> giveRole(GuildMember member, Role role) {
		CompletableFuture<GuildMember> future = new CompletableFuture<GuildMember>();
		this.makeRequest(Endpoints.guildMemberRole(member.guild.id, member.id, role.id), Methods.PUT, true)
				.thenAcceptAsync(action -> {
					future.complete(member);
				});
		return future;
	...
	public CompletableFuture<GuildMember> loadGuildMember(Guild guild, String memberID) {
		CompletableFuture<GuildMember> future = new CompletableFuture<GuildMember>();
		this.makeRequest(DLUtil.Endpoints.guildMember(guild.id, memberID), DLUtil.Methods.GET, true)
				.thenAcceptAsync(action -> {
	...
	public CompletableFuture<Message> sendMessage(ITextChannel channel, String content, RichEmbed embed,
		...
		this.makeRequest(DLUtil.Endpoints.messages(channel.getID()), DLUtil.Methods.POST, true,
				new SendableMessage(content, embed, attachment, file)).thenAcceptAsync(action -> {
	...
	public CompletableFuture<User> setAvatar(String avatar) {
		...
		try {
			...
			this.makeRequest(DLUtil.Endpoints.currentUser, DLUtil.Methods.PATCH, true,
	...
	public CompletableFuture<GuildMember> setNick(GuildMember member, String nick) {
		CompletableFuture<GuildMember> future = new CompletableFuture<GuildMember>();
		String endpoint = member.id == this.loader.user.id ? DLUtil.Endpoints.guildNick(member.guild.id)
				: DLUtil.Endpoints.guildMember(member.guild.id, member.id);
		this.makeRequest(endpoint, DLUtil.Methods.PATCH, true, new JSONObject().put("nick", nick))
				.thenAcceptAsync(action -> {
					member.nick = nick;
					future.complete(member);
				});
	...
	public CompletableFuture<GuildMember> takeRole(GuildMember member, Role role) {
		CompletableFuture<GuildMember> future = new CompletableFuture<GuildMember>();
		this.makeRequest(Endpoints.guildMemberRole(member.guild.id, member.id, role.id), Methods.DELETE, true)
				.thenAcceptAsync(action -> {
					future.complete(member);
				});
		return future;
	...
	public CompletableFuture<Role> createRole(Guild guild, String name, int permissions, int color, boolean hoist,
			boolean mentionable) {
