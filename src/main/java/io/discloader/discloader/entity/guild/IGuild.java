...
public interface IGuild extends ISnowflake, ICreationTime {
	...
	/**
	 * Begin a prune operation. Requires the {@link Permissions#KICK_MEMBERS KICK_MEMBERS} permission.
	 * 
	 * @return A Future that completes with the number of member kicked during the prune operation if successful.
	 * @throws PermissionsException if the current user doesn't have the {@link Permissions#KICK_MEMBERS} permission.
	 */
	 ...
	 * @return A Future that completes with the number of member kicked during the prune operation if successful.
	 * @throws PermissionsException if the current user doesn't have the {@link Permissions#KICK_MEMBERS KICK_MEMBERS} permission.
	 */
	 ...
	 * @return A future that completes with a new {@link IRole} Object if successful.
	 * @since 0.0.3
	 */
	 ...
	 * @return A future that completes with a new {@link IGuildVoiceChannel} Object if successful.
	 */
	 ...
	 * @param category
	 * @param overwrites The channel's overwrites
	 * @return A CompletableFuture that completes with a new {@link IGuildVoiceChannel} Object if successful.
	 */
	 ...
	 * 
	 * @param name The name of the channel
	 * @param bitRate The channel's bitrate in kilobits per second (kbps). <code>1kb</code> is equal to <code>1000bits (125Bytes)</code>.
	 *            The bitrate is normalized to be within the range of <code>8 to 128kbps</code>
	 * @param userLimit The channel's userlimit with a maximum value of <code>99 users</code>. A userLimit of <code>0</code> signifies no
	 *            limit.
	...
	/**
	 * Requests the Gateway to send a {@link GuildMembersChunkEvent} containing this {@link IGuild guild's} members
	 * 
	 * @return A Map of {@link IGuildMember} objects indexed by {@link IGuildMember#getID()}
	 */
	 ...
	 * 
	 * @return the {@link IGuildTextChannel} with the highest position that your {@link Permissions} allow you to see.
	 */
	...
	/**
	 * same as {@link #getPruneCount(int days)} but only grabs one days worth. Requires the {@link Permissions#KICK_MEMBERS} permission.
	 * 
	 * @return A Future that completes with the number of members that would be kicked in a prune operation
	 */
	CompletableFuture<Integer> getPruneCount();
	
	/**
	 * Returns the number of members that would be removed in a prune operation. Requires the {@link Permissions#KICK_MEMBERS} permission.
	 * 
	...
	/**
	 * returns a Map of the guild's members' voice states. Indexed by {@link GuildMember#getID()}
	 * 
	...
	/**
	 * Checks if the {@link #getCurrentMember() current member} has the specified permissions in <code>this</code> {@link IGuild guild}.
	 * <br>
	 * <br>
	 * This check is <strong>explicit</strong> if the {@link #getCurrentMember() current member} is not the {@link IGuild guild's}
	 * {@link #getOwner() owner}.
	 * 
	 ...
	 * 
	 * @return {@code if} {@link #getMemberCount()} is greater than or equal to 250, {@code false} otherwise.
	 */
	boolean isLarge();
	
	/**
	 * Checks whether or not the {@link IUser user} you are logged in as is the owner of the {@link IGuild guild}
	 * 
	 * @return {@code true} if {@link IGuildMember#getID() getCurrentMember().getID()} is equal to {@link #getOwnerID()}, false otherwise.
	 */
	boolean isOwner();
	
	/**
	 * Checks if a {@link IGuildMember member} is the {@link #getOwner() owner} of the {@link IGuild guild}.
	 * 
	 * @param member The member to check if they own the {@link IGuild guild}.
	 * @return {@code true} if {@link IGuildMember#getID() member.getID()} is equal to {@link #getOwnerID() this.getOwnerID()}, false
	 *         otherwise.
	 */
	...
	/**
	 * Kicks a {@link IGuildMember} from the {@link IGuild guild} if the {@link #getCurrentMember() current member} has the
	 ...
	 * 
	 * @param member The {@link IGuildMember member} to kick from the guild.
	 * @param reason The reason for kicking the user to be shown in the {@link IAuditLog audit log}.
	 * @see Permissions
	 * @return A CompletableFuture that completes with {@code this} if successful.
	 * @throws PermissionsException Thrown if the current user doesn't have the {@link Permissions#KICK_MEMBERS} permission.
	 * 
	 ...
	 * 
	 * @return A CompletableFuture that completes with the {@link IGuild guild} that has been left if successful.
	 */
	...
	/**
	 * @param member The member to set as the {@link IGuild guild's} {@link #getOwner() owner}.
	 * @return A Future that completes with the new {@link #getOwner() owner} of the {@link IGuild guild} if successful. {@link IGuild this}
	 *         if successful.
