...
public final class DLUtil {
	...
	public static final byte[] readAllBytes(File file) throws IOException {
		...
		try {
			...
			String loc = path.length == 2 ? path[1] : path[0];
		...
		if (is == null)
			throw new IOException("Some thing went wrong reading the file");
	...
	// public static byte[] readAllBytes(InputStream is, File file) {
	//
	// }
