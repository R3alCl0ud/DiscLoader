        ...
        if (shard != -1) {
            ...
            DiscLoader.addEventHandler(new EventListenerAdapter() {
                ...
                public void GuildBanAdd(GuildBanAddEvent e) {
                    ...
                    e.guild.getDefaultChannel().sendMessage(String.format("%s was banned from the server", e.member.toString()));
                }
                ...
                public void MessageCreate(MessageCreateEvent e) {
                    LOGGER.fine(String.format("Author: %s#%s, Channel: %s", e.message.author.username, e.message.author.discriminator, e.message.channel.getID()));
                }
