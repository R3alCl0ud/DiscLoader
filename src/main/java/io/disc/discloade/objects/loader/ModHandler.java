...
public class ModHandler {
	...
	public void beginLoader(String customAnnotationPath) {
		if (customAnnotationPath != null)
			this.ann_class = customAnnotationPath;
					...
					while (this.e.hasMoreElements()) {
						...
						try {
							...
							for (int j = 0, n = this.instance.getDeclaredMethods().length; j < n; j++) {
								System.out.println("\tMethod Name: " + this.instance.getDeclaredMethods()[i].getName()
										+ " Method Num: " + (j + 1) + " out of " + n);
