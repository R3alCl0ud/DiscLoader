...
public class DiscRESTQueue {
	...
	public void handle() {
		...
		final DiscRESTQueue this_arg = this; 
		request.asStringAsync(new Callback<String>() {
			...
			public void completed(HttpResponse<String> response) {
				...
				try {
					this_arg.timeDifference = Date.from(Instant.now()).getTime() - df.parse(headers.get("Date").get(0)).getTime();
				...
				if (this_arg.remaining == 0) {
					...
					this_arg.loader.timer.schedule(wait, this_arg.resetTime - System.currentTimeMillis() + this_arg.timeDifference + 500);
				} else {					
					this_arg.waiting = false;
					this_arg.handle();
				}
