...
public class DiscREST {
	...
	public CompletableFuture<GuildMember> setNick(GuildMember member, String nick) {
		...
		String endpoint = member.id == this.loader.user.id ? constants.Endpoints.guildNick(member.guild.id) : constants.Endpoints.guildMember(member.guild.id, member.id);
		this.makeRequest(endpoint, constants.Methods.PATCH, true, new JSONObject().put("nick", nick)).thenAcceptAsync(action -> {
			member.nick = nick;
	...
	public CompletableFuture<Guild> modifyGuild(Guild guild, JSONObject data) {
		...
		this.makeRequest(constants.Endpoints.guild(guild.id), constants.Methods.PATCH, true, data).thenAcceptAsync(action -> {
	...
	public CompletableFuture<GuildMember> loadGuildMember(Guild guild, String memberID) {
		CompletableFuture<GuildMember> future = new CompletableFuture<GuildMember>();
		this.makeRequest(constants.Endpoints.guildMember(guild.id, memberID), constants.Methods.GET, true).thenAcceptAsync(action -> {
